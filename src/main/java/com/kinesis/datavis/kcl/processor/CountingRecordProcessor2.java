 ...
 */
public class CountingRecordProcessor2<T, C> implements IRecordProcessor {
    ...
    private FlushBuffer<byte[]> s3buffer;
    @Getter
    public BlockingQueue<Mapping> mappingsBuff = new LinkedBlockingQueue<>(60000);
     ...
     */
    public CountingRecordProcessor2(CountingRecordProcessorConfig config,
                                    Class<T> recordType,
    ...
    public void processRecords(List<Record> records, IRecordProcessorCheckpointer checkpointer) {
        for (Record r : records) {
            ...
            try {
                ...
                BidResponse resp = (BidResponse) rec;

                s3buffer.consumeRecord(r.getData().array(), r.getData().array().length, r.getSequenceNumber());

                mappingsBuff.add(new Mapping(resp.getId(), resp.getBannerId(), resp.getAudienceId(), new Timestamp(System.currentTimeMillis())));
        ...
        if (s3buffer.shouldFlush()) {
            emit(s3buffer.getRecords());
        }
    ...
    private void emit(List<byte[]> emitItems) {
        ...
        try {
            for (int numTries = 0; numTries < 3; numTries++) {
                unprocessed = emitter.emit(new UnmodifiableBuffer<byte[]>(s3buffer));
            }
